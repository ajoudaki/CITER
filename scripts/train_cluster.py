...
# --- GLOBALS -------------------------------------------------------
global_rank = 0  # <<< NEW (ensure defined before any function reads it)
...
class CLSPoolingEncoder(nn.Module):
    ...
    def __init__(self, model_name: str, hidden_dim: int, output_dim: int, use_gradient_checkpointing: bool = False, quantization_config=None):
        super().__init__()
        # Pass quantization_config if it exists; under DDP bind to this process's GPU
        model_kwargs = {
            "quantization_config": quantization_config,
        }
        if quantization_config:  # <<< CHANGED
            try:
                model_kwargs["device_map"] = {"": torch.cuda.current_device()}  # <<< CHANGED
            except Exception:
                model_kwargs["device_map"] = {"": 0}  # <<< CHANGED
...
def prepare_validation_data(val_dataset, device, distributed, rank):
    ...
    else:  # non-distributed path (compute locally)  # <<< CHANGED
...
# Training Function
# ===================================================================
def train(cfg: DictConfig, rank: int = 0, world_size: int = 1, local_rank: int = 0, distributed: bool = False):  # <<< CHANGED
    """Main training function with learning rate scheduler."""
    if distributed and torch.cuda.is_available():  # <<< CHANGED
        torch.cuda.set_device(local_rank)  # <<< CHANGED
        device = torch.device(f'cuda:{local_rank}')  # <<< CHANGED
    else:
    ...
    if distributed:
        model = DDP(model, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)  # <<< CHANGED
...
def main(cfg: DictConfig):
    ...
    # wandb enablement purely controlled by Hydra config/overrides
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        dist.init_process_group(backend="nccl", init_method="env://")  # <<< CHANGED
        rank = dist.get_rank()
        world_size = dist.get_world_size()
        local_rank = int(os.environ.get("LOCAL_RANK", 0))  # <<< CHANGED
        print(f"Running in distributed mode. Rank: {rank}, Local Rank: {local_rank}, World Size: {world_size}")  # <<< CHANGED
        train(cfg, rank, world_size, local_rank=local_rank, distributed=True)  # <<< CHANGED
